...
ipcMain.handle('launch-minecraft', async (event, options: LaunchOptions) => {
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      // 9. Add LWJGL (required by Minecraft and mods)
      const lwjglVersion = '3.3.3'
      const lwjglLibs = [
        `org.lwjgl:lwjgl:${lwjglVersion}`,
        `org.lwjgl:lwjgl-glfw:${lwjglVersion}`,
      ...
      if (!fs.existsSync(nativesDir)) {
        ...
        event.sender.send('launch-log', `[Launcher] Extracting native libraries to ${nativesDir}...`)
        ...
        for (const nativeJar of nativeLibs) {
          ...
          if (fs.existsSync(nativePath)) {
            try {
              ...
              event.sender.send('launch-log', `[Launcher] ⚠️ Failed to extract ${nativeJar}: ${(err as Error).message}`)
            }
          }
        }
      }
