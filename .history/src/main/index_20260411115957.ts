...
const downloadMinecraftClient = async (gameDir: string, mcVersion: string, onProgress?: (msg: string) => void): Promise<boolean> => {
  try {
    ...
    if (fs.existsSync(mcJarPath)) return true
    ...
    const versionData = versionRes.data as { downloads?: { client?: { url: string; sha1: string } } }
...
ipcMain.handle('launch-minecraft', async (event, options: LaunchOptions) => {
  try {
    ...
    if (options.modLoader === 'forge' || options.modLoader === 'neoforge') {
      ...
      // Download Minecraft client jar (required for Fabric)
      ...
      // Download Minecraft client jar (required for Quilt)
      const mcDownloaded = await downloadMinecraftClient(
        options.gameDir,
        options.mcVersion,
        (msg) => event.sender.send('launch-log', msg)
    ...
    await new Promise<void>((resolve, reject) => {
      const javaPath = options.javaPath || getAutoJavaPath() || 'java'
      const versionId = loaderOpts.version?.number || options.mcVersion
      ...
      // Add Minecraft client jar (required for Fabric)
      ...
      // Add version jar (Fabric/Quilt loader)
      const versionJar = path.join(versionDir, `${versionId}.jar`)
      if (fs.existsSync(versionJar)) {
        classpath.push(versionJar)
        event.sender.send('launch-log', `[Launcher] Added version jar: ${versionJar}`)
      } else {
        event.sender.send('launch-log', `[Launcher] ⚠️ Version jar not found: ${versionJar}`)
      }

      // Add libraries from profile (use name to construct path)
      if (versionJson.libraries) {
        for (const lib of versionJson.libraries) {
          if (lib.name) {
            const { path: mavenPath } = parseMavenPath(lib.name)
            const libPath = path.join(libDir, mavenPath)
            if (fs.existsSync(libPath)) {
              classpath.push(libPath)
            } else {
              event.sender.send('launch-log', `[Launcher] ⚠️ Library not found: ${lib.name}`)
            }
          }
        }
      }
      ...
      event.sender.send('launch-log', `[Launcher] Classpath entries: ${classpath.length}`)
