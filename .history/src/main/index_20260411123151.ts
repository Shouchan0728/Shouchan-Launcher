...
ipcMain.handle('launch-minecraft', async (event, options: LaunchOptions) => {
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      // 3. Add libraries from profile (use name to construct path)
      ...
      const slf4jLibs = [
        path.join(libDir, 'org/slf4j/slf4j-api/2.0.9/slf4j-api-2.0.9.jar'),
        path.join(libDir, 'org/slf4j/slf4j-simple/2.0.9/slf4j-simple-2.0.9.jar')
      ]
      for (const slf4jPath of slf4jLibs) {
        if (fs.existsSync(slf4jPath)) {
          classpath.push(slf4jPath)
          libCount++
          event.sender.send('launch-log', `[Launcher] Added slf4j: ${path.basename(slf4jPath)}`)
        }
      }
