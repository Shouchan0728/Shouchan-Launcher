...
ipcMain.handle('launch-minecraft', async (event, options: LaunchOptions) => {
  try {
    ...
    // Event listeners must be set BEFORE calling launch()
    ...
    // Launch and wait for initial start
    await new Promise<void>((resolve, reject) => {
      const timeout = setTimeout(() => reject(new Error('起動タイムアウト (30秒)')), 30000)

      launcher.on('data', (data: string) => {
        if (data.includes('Setting user:') || data.includes('Loaded Minecraft') || data.includes('Backend library')) {
