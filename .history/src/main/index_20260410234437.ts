...
ipcMain.handle('update-modpack-by-id', async (event, id: string, modpackDir: string) => {
  try {
    ...
    if (!Array.isArray(files) || files.length === 0) {
      return { success: false, error: 'サーバーにファイルがまだアップロードされていません。開発者メニューから一括アップロードを行ってください。' }
    }
    let completed = 0
    for (const file of files) {
      const filePath = path.join(modpackDir, file.path)
      ...
      try {
        ...
        if (status === 404) {
          return { success: false, error: `ファイルが見つかりません: ${file.path}\nサーバーのnginx設定を確認してください。` }
        }
        throw fileErr
      }
      completed++
...
ipcMain.handle('dev-upload-directory', async (event, modpackId: string, localDir: string, serverBasePath: string) => {
  try {
    ...
    scanDir(localDir)
    const uploadUrl = modpackId === 'default'
      ? `${MODPACK_SERVER_URL}/admin/upload`
      : `${MODPACK_SERVER_URL}/admin/modpacks/${modpackId}/upload`
    let completed = 0
    for (const file of files) {
