...
const installFabricLoader = async (gameDir: string, mcVersion: string, loaderVersion: string): Promise<boolean> => {
  try {
    const versionId = `fabric-loader-${mcVersion}-${loaderVersion}`
    const versionDir = path.join(gameDir, 'versions', versionId)
    const versionJsonPath = path.join(versionDir, `${versionId}.json`)

    if (fs.existsSync(versionJsonPath)) return true
...
const installQuiltLoader = async (gameDir: string, mcVersion: string, loaderVersion: string): Promise<boolean> => {
  try {
...
ipcMain.handle('launch-minecraft', async (event, options: LaunchOptions) => {
  try {
    ...
    if (options.modLoader === 'forge' || options.modLoader === 'neoforge') {
      ...
      event.sender.send('launch-log', `[Fabric] Installing loader ${resolvedLoaderVersion}...`)
      const installed = await installFabricLoader(options.gameDir, options.mcVersion, resolvedLoaderVersion)
      if (!installed) {
      ...
      event.sender.send('launch-log', `[Quilt] Installing loader ${resolvedLoaderVersion}...`)
      const installed = await installQuiltLoader(options.gameDir, options.mcVersion, resolvedLoaderVersion)
      if (!installed) {
