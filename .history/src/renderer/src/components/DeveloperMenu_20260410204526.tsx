...
import { ModpackInfo, NewsItem, ServerModpack } from '../types'

type DevTab = 'files' | 'info' | 'news' | 'modpacks' | 'mc-auth'
...
export default function DeveloperMenu({ mcUsername, onMcUsernameChange }: DeveloperMenuProps): React.JSX.Element {
  const [tab, setTab] = useState<DevTab>('upload-all')
  ...
  const [newMpForm, setNewMpForm] = useState({ name: '', version: '1.0.0', mcVersion: '1.20.1', forgeVersion: '', description: '' })
  ...
  const handleCreateModpack = async () => {
    ...
    const res = await window.api.devCreateModpack({
      ...
      forgeVersion: newMpForm.forgeVersion.trim() || undefined,
      description: newMpForm.description.trim() || undefined
    })
    if (res.success) {
      showStatus(`ModPack「${newMpForm.name}」を作成しました`, 'ok')
      setNewMpForm({ name: '', version: '1.0.0', mcVersion: '1.20.1', forgeVersion: '', description: '' })
  ...
  const tabs: { id: DevTab; label: string; icon: React.ReactNode }[] = [
    ...
    { id: 'info', label: 'ModPack情報(旧)', icon: <FileText size={14} /> },
      ...
      <div className="flex-1 overflow-y-auto p-5">
        ...
        {tab === 'upload-all' && (
          <div className="flex flex-col gap-5 max-w-lg">
            <div className="rounded-xl bg-yellow-500/5 border border-yellow-500/20 p-4 text-sm text-yellow-300">
              <p className="font-semibold mb-1">一括アップロード</p>
              <p className="text-xs text-yellow-400/70">ローカルフォルダを選択すると、全ファイルのハッシュ計算・アップロード・マニフェスト更新を自動で行います。</p>
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1">ModPackフォルダ（ローカル）</label>
              <div className="flex gap-2">
                <input type="text" value={bulkDir} onChange={(e) => setBulkDir(e.target.value)} placeholder="C:\\modpack-folder" readOnly
                  className="flex-1 rounded-lg bg-[#1a1a2e] border border-white/10 px-3 py-2 text-sm text-white placeholder-gray-600 outline-none" />
                <button onClick={async () => { const d = await window.api.selectDirectory(); if (d) setBulkDir(d) }}
                  className="flex items-center gap-1.5 rounded-lg bg-[#252535] px-3 py-2 text-sm text-gray-300 hover:bg-[#303045] flex-shrink-0">
                  <FolderOpen size={14} />
                </button>
              </div>
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1">バージョン</label>
              <input type="text" value={bulkVersion} onChange={(e) => setBulkVersion(e.target.value)} placeholder="v1.0.1"
            ...
            {bulkUploading && (
              <div>
                <div className="flex justify-between text-xs text-gray-400 mb-1">
                  <span className="truncate max-w-xs">{bulkProgress.file}</span>
                  <span className="flex-shrink-0 ml-2">{bulkProgress.current} / {bulkProgress.total}</span>
                </div>
                <div className="w-full bg-[#1a1a2e] rounded-full h-2">
                  <div className="bg-yellow-500 h-2 rounded-full transition-all" style={{ width: bulkProgress.total > 0 ? `${Math.round(bulkProgress.current / bulkProgress.total * 100)}%` : '0%' }} />
                </div>
              </div>
            )}
            {bulkDone && !bulkUploading && (
              <div className="flex items-center gap-2 text-green-400 text-sm">
                <CheckCircle size={16} /> アップロード完了！
              </div>
            )}
            <button onClick={handleBulkUpload} disabled={!bulkDir || !bulkVersion.trim() || bulkUploading}
              className="flex items-center justify-center gap-2 rounded-xl bg-yellow-600 hover:bg-yellow-500 disabled:opacity-40 px-5 py-3 text-sm font-bold transition-colors">
              {bulkUploading ? <><Loader2 size={15} className="animate-spin" />アップロード中...</> : <><Zap size={15} />一括アップロード開始</>}
            </button>
          </div>
        )}
