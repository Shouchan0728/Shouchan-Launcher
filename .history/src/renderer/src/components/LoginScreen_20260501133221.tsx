...
import { Mail, Lock, Eye, EyeOff, Loader2, CheckCircle, Minus, X } from 'lucide-react'
...
export default function LoginScreen({ onLogin }: LoginScreenProps): React.JSX.Element {
  const [step, setStep] = useState<'shouchan' | 'shouchan-code' | 'microsoft'>('shouchan')
  ...
  const handleVerifyCode = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    const res = await window.api.accountLoginVerify({ pendingToken, code: verificationCode.trim() })
    setLoading(false)
      ...
      <div className="w-[340px] rounded-2xl bg-[#1a1a2e] border border-white/5 p-8 shadow-2xl">
        ...
        <div className="flex flex-col items-center gap-2 mb-7">
          ...
          <p className="text-xs text-gray-500">
            {step === 'microsoft' ? 'Minecraftアカウントを連携' : 'アカウントにログイン'}
          </p>
        </div>
        ...
        {step === 'shouchan-code' && (
          <form onSubmit={handleVerifyCode} className="flex flex-col gap-4">
            ...
            {error && <p className="text-xs text-red-400 bg-red-500/10 rounded-lg px-3 py-2">{error}</p>}
            <button type="submit" disabled={loading} className="flex items-center justify-center gap-2 rounded-lg bg-blue-600 hover:bg-blue-500 disabled:opacity-50 py-2.5 text-sm font-semibold text-white transition-colors mt-1">
              {loading ? <><Loader2 size={14} className="animate-spin" />確認中...</> : '確認してログイン'}
            </button>
            <button type="button" onClick={() => { setStep('shouchan'); setError(''); setVerificationCode('') }} className="text-xs text-gray-600 hover:text-gray-400 transition-colors">
              ← 戻る
            </button>
          </form>
        )}
