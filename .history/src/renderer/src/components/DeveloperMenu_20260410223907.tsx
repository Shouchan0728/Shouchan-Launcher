...
export default function DeveloperMenu({ mcUsername, onMcUsernameChange }: DeveloperMenuProps): React.JSX.Element {
  ...
  const [folderUploadProgress, setFolderUploadProgress] = useState({ current: 0, total: 0, file: '' })
  ...
  const handleUpload = async () => {
    ...
    setUploading(true)
    const serverPath = currentPath ? `${currentPath}/${fileName}` : fileName
  ...
  const handleFolderUpload = async () => {
    ...
    const serverBasePath = currentPath ? `${currentPath}/${folderName}` : folderName
    setFolderUploading(true)
    setFolderUploadProgress({ current: 0, total: 0, file: '' })
    const cleanup = window.api.onUploadProgress((data) => setFolderUploadProgress(data))
    const res = await window.api.devUploadDirectory(selectedModpackForFiles, dir, serverBasePath)
    cleanup()
    setFolderUploading(false)
    if (res.success) {
      showStatus(`「${folderName}」を ${serverBasePath}/ としてアップロードしました (${res.count} ファイル)`, 'ok')
      loadFiles()
    } else {
      showStatus(res.error || 'アップロード失敗', 'error')
    }
                  ...
                  <div className="flex items-center gap-1.5 bg-[#0d0d14] rounded-lg border border-white/8 px-2 py-1.5 flex-shrink-0">
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0 ml-1">
                      <button
                        ...
                        title={`ここへアップロード (${currentPath || 'ルート'})`}
                      >
                        <Upload size={11} />
                        {uploading ? 'アップロード中...' : 'ここへアップロード'}
                      </button>
                      <button
                        onClick={() => loadFiles()}
                        disabled={loading}
