...
type DevTab = 'files' | 'info' | 'news' | 'upload-all' | 'mc-auth'
...
export default function DeveloperMenu({ mcUsername, onMcUsernameChange }: DeveloperMenuProps): React.JSX.Element {
  ...
  const handleBulkUpload = async () => {
    if (!bulkDir || !bulkVersion.trim()) return
    setBulkUploading(true)
    setBulkDone(false)
    setBulkProgress({ current: 0, total: 0, file: '' })
    const cleanup = window.api.onUploadProgress((data) => setBulkProgress(data))
    const res = await window.api.devUploadModpackDir(bulkDir, bulkVersion.trim())
    cleanup()
    setBulkUploading(false)
    if (res.success) {
      setBulkDone(true)
      showStatus(`ModPack v${bulkVersion} のアップロードが完了しました`, 'ok')
    } else {
      showStatus(res.error || 'アップロード失敗', 'error')
    }
  }
  ...
  const tabs: { id: DevTab; label: string; icon: React.ReactNode }[] = [
    { id: 'upload-all', label: '一括アップロード', icon: <Zap size={14} /> },
