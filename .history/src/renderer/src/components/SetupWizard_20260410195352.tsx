...
export default function SetupWizard({ onComplete }: SetupWizardProps): React.JSX.Element {
  ...
  const handleStep0Next = async () => {
    ...
    if (accountMode === 'register') {
      ...
      const res = await window.api.accountRegister({ username: data.launcherUsername, email: data.email, password: data.password })
      setAccountLoading(false)
      if (!res.success || !res.account) { setAccountError(res.error || '登録に失敗しました'); return }
      update({ resolvedAccount: res.account })
    } else {
      if (!data.email.trim()) { setAccountError('メールアドレスを入力してください'); return }
      if (!data.password) { setAccountError('パスワードを入力してください'); return }
      setAccountLoading(true)
      const res = await window.api.accountLogin({ email: data.email, password: data.password })
      setAccountLoading(false)
      if (!res.success || !res.account) { setAccountError(res.error || 'ログインに失敗しました'); return }
      update({ resolvedAccount: res.account, launcherUsername: res.account.username })
    }
    setStep(1)
  }
  ...
  const handleComplete = async () => {
    ...
    const defaultProfile = {
      ...
      closeOnLaunch: data.closeOnLaunch
    }
    await window.api.setProfiles([defaultProfile])
    await window.api.setStore('activeProfileId', 'default')
    ...
    <div className="flex flex-col h-screen bg-[#111117] text-white select-none overflow-hidden">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex rounded-lg bg-[#1a1a2e] p-1 gap-1">
                {(['register', 'login'] as const).map((mode) => (
                  <button
                    key={mode}
                    onClick={() => { setAccountMode(mode); setAccountError('') }}
              ...
              <div>
                ...
                <button onClick={handleInstallJava} disabled={javaStatus === 'loading' || javaStatus === 'done'}
                  ...
                  {javaStatus === 'loading' ? <><Loader2 size={14} className="animate-spin" />ダウンロード中... {javaProgress}%</>
                    : javaStatus === 'done' ? <><CheckCircle size={14} />Java 17 インストール済み</>
      ...
      <div className="flex items-center justify-between px-5 py-3 bg-[#0d0d14] border-t border-white/5 flex-shrink-0">
        ...
        {step < 5 ? (
          ...
          >
            {accountLoading ? <><Loader2 size={14} className="animate-spin" />処理中...</> : <>次へ <ChevronRight size={16} /></>}
          </button>
        ) : (
