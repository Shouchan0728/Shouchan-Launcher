...
export default function HomeView({
  ...
  const handleSwitchProfile = async (profileId: string) => {
    ...
    await window.api.setStore('activeProfileId', profileId)
    const p = profiles.find((x) => x.id === profileId)
    if (p?.gameDir) setGameDir(p.gameDir)
  ...
  const handleAddProfile = async () => {
    ...
    setProfiles(updated)
    await window.api.setProfiles(updated)
    setNewProfileName('')
    setNewProfileDir('')
  ...
  const handleDeleteProfile = async (profileId: string) => {
    ...
    setProfiles(updated)
    await window.api.setProfiles(updated)
    if (activeProfileId === profileId) handleSwitchProfile(updated[0].id)
  }

  const handleLaunch = async () => {
    if (launchStatus === 'running') return

    let dir = activeProfile?.gameDir || gameDir
    if (!dir) {
      ...
      setGameDir(dir)
      if (activeProfile) {
        const updated = profiles.map((p) => p.id === activeProfileId ? { ...p, gameDir: dir } : p)
        setProfiles(updated)
        await window.api.setProfiles(updated)
      }
    }

    const maxMem = activeProfile?.maxMemory || ((await window.api.getStore('settings.maxMemory')) as string) || '4G'
    const minMem = activeProfile?.minMemory || ((await window.api.getStore('settings.minMemory')) as string) || '2G'
    const javaPath = activeProfile?.javaPath || ((await window.api.getStore('settings.javaPath')) as string) || ''
    const closeOnLaunch = activeProfile?.closeOnLaunch ?? ((await window.api.getStore('settings.closeOnLaunch')) as boolean) ?? false
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {showProfileModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setShowProfileModal(false)}>
            <div className="w-[360px] rounded-2xl bg-[#1a1a2e] border border-white/10 p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-sm">ModPackプロファイル</h3>
                <button onClick={() => setShowProfileModal(false)} className="text-gray-500 hover:text-white"><X size={16} /></button>
              </div>
              <div className="flex flex-col gap-2 mb-4 max-h-48 overflow-y-auto">
                {profiles.map((p) => (
                  <div key={p.id} className={`flex items-center gap-3 rounded-lg px-3 py-2 cursor-pointer transition-colors ${p.id === activeProfileId ? 'bg-green-600/20 border border-green-500/30' : 'bg-[#111117] border border-white/5 hover:border-white/20'}`}
                    ...
                    {profiles.length > 1 && p.id !== activeProfileId && (
                      <button onClick={(e) => { e.stopPropagation(); handleDeleteProfile(p.id) }} className="text-red-500/50 hover:text-red-400 flex-shrink-0">
                        <Trash2 size={13} />
                      </button>
                    )}
                  </div>
              ...
              <div className="border-t border-white/5 pt-4">
                <p className="text-xs text-gray-500 mb-2">新規プロファイル</p>
                <div className="flex flex-col gap-2">
                  <input type="text" value={newProfileName} onChange={(e) => setNewProfileName(e.target.value)} placeholder="プロファイル名"
                    className="w-full rounded-lg bg-[#111117] border border-white/10 px-3 py-2 text-sm text-white placeholder-gray-600 outline-none focus:border-green-500/50" />
                  <div className="flex gap-2">
                    <input type="text" value={newProfileDir} onChange={(e) => setNewProfileDir(e.target.value)} placeholder="ゲームディレクトリ（任意）"
                      className="flex-1 rounded-lg bg-[#111117] border border-white/10 px-3 py-2 text-sm text-white placeholder-gray-600 outline-none focus:border-green-500/50" />
                    <button onClick={async () => { const d = await window.api.selectDirectory(); if (d) setNewProfileDir(d) }}
                      className="rounded-lg bg-[#252535] px-3 py-2 text-gray-400 hover:text-white transition-colors flex-shrink-0"><FolderOpen size={14} /></button>
                  </div>
        ...
        <div className="mb-6 rounded-xl bg-gradient-to-br from-[#1a2a1a] to-[#0d1a0d] border border-green-900/30 p-6">
          <div className="flex flex-col items-center gap-3">
            ...
            <button onClick={() => setShowProfileModal(true)}
              className="flex items-center gap-2 rounded-lg bg-[#111117]/60 border border-white/10 px-4 py-1.5 text-xs text-gray-300 hover:text-white hover:border-white/20 transition-colors w-80">
              <span className="flex-1 text-left truncate">📦 {activeProfile?.name || 'Default'}</span>
