...
import { UserAccount, Stats, NewsItem, ModpackInfo, LaunchStatus } from '../types'
...
export default function HomeView({
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const [serverResult, newsResult, modpackResult] = await Promise.allSettled([
  ...
  const handleUpdateModpack = async () => {
    ...
    const result = await window.api.updateModpack(gameDir, account.username)
    if (result.success) {
      const newVersion = modpackInfo?.version || 'latest'
    ...
    <div className="flex h-full overflow-hidden">
      <div className="flex-1 overflow-y-auto p-6">
        <div className="mb-4 flex items-center gap-2 text-gray-300">
          <Gamepad2 size={16} />
          <span className="text-sm">ようこそ、{account.username}さん</span>
        </div>
      ...
      <div className="w-56 flex-shrink-0 overflow-y-auto border-l border-white/5 p-4 flex flex-col gap-4">
        ...
        <div className="rounded-lg bg-[#1a1a2e] border border-white/5 p-3">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-gray-300">自動ログインしました: {account.username}</span>
          </div>
