          ...
          <div className="flex flex-col gap-4 max-w-2xl">
            {news.map((item) => (
              <div key={item.id} className="rounded-xl bg-[#1a1a2e] border border-white/5 p-4">
                ...
                <input type="datetime-local" value={item.date ? item.date.slice(0, 16) : ''}
                  onChange={(e) => setNews((prev) => prev.map((n) => n.id === item.id ? { ...n, date: new Date(e.target.value).toISOString() } : n))}
